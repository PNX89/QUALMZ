...
```bash
git clone https://github.com/PNX89/QUALMZ.git && cd QUALMZ
uv sync --dev
...
- the budget and the grant, against a real PostgreSQL
- bad luck or a bug, with a number in it
- three layers and a gate that refuses a different data version
- a promotion that stops for a named human, and one that does not
...
https://pnx89.github.io/QUALMZ, come from a real run whose output is committed under
